"""FLAN-T5 local adapter — sends requests to the Worker container."""
...
class FlanT5Adapter(BaseAdapter):
    """Adapter for FLAN-T5 models running in the local Worker container."""
    ...
    async def _call_worker(self, request: ExtractionRequest) -> dict:
        ...
        try:
            ...
            raise ExtractionError(f"Local worker returned HTTP {exc.response.status_code}.") from exc
    ...
    @staticmethod
    def _to_standardized_extraction(payload: dict, request: ExtractionRequest) -> StandardizedExtraction:
